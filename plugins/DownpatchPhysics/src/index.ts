...
api.net.onLoad(() => {
    ...
    function i9(g: any) {
        ...
        if (t !== ee())
            return;
        let e = WA(t);
        e && e.physics.lastClassDeviceActivationId !== k.me.classDesigner.lastClassDeviceActivationId && (e.physics.lastClassDeviceActivationId = k.me.classDesigner.lastClassDeviceActivationId,
        k.me.classDesigner.activeClassDeviceId = k.me.classDesigner.lastActivatedClassDeviceId)
    }
    ...
    function z5(g: any) {
        ...
        if (!e || e.physics.projectileHitForcesHistory.has(t.id))
            return;
        e.physics.projectileHitForcesHistory.set(t.id, Date.now()),
        e.physics.justAppliedProjectileHitForces.add(t.id);
        let I = Z5({
            projectile: t,
            character: e
        });
    ...
    function Iy(g: any) {
        ...
        Ve().world.intersectionsWithShape(e.rigidBody.translation(), 0, new RAPIER.Capsule(s.height,s.radius), (B: any) => {
            ...
            , Q = r.find(n) as any;
            return Q != null && Q.character || (l = Q == null ? void 0 : Q.terrain) != null && l.removed || n && (g.reset || I.has(n)) && o.add(n),
            !0
        }
        ),
    ...
    function _5(g: any) {
        ...
        , I = WA(t);
        if (!I || (I.physics.state.jump.actuallyJumped = !1,
        I.physics.state.jump.jumpTicks += 1,
        e === void 0))
            return;
        if (I.physics.state.grounded) {
            ...
            I.physics.state.jump.jumpsLeft = r,
            I.physics.state.jump.jumpCounter = 0
        }
    ...
    function j5(g: any, t: any) {
        ...
        if (g.physics.state.jump.isJumping) {
            ...
            n = Math.max(d, g.physics.state.jump.xVelocityAtJumpStart),
            n = Math.min(n, we.platformerGroundSpeed * E)
        }
        ...
        let c = l !== 0;
        if (B !== g.physics.state.movement.direction && (c && g.physics.state.movement.xVelocity !== 0 && (g.physics.state.movement.xVelocity = 0),
        g.physics.state.movement.accelerationTicks = 0,
        g.physics.state.movement.direction = B),
        g.physics.state.movement.xVelocity !== l) {
            g.physics.state.movement.accelerationTicks += 1;
            let d = 0
            , y = 0;
            g.physics.state.grounded ? c ? (d = we.movement.ground.accelerationSpeed,
            y = we.movement.ground.maxAccelerationSpeed) : d = we.movement.ground.decelerationSpeed : c ? (d = we.movement.air.accelerationSpeed,
            y = we.movement.air.maxAccelerationSpeed) : d = we.movement.air.decelerationSpeed;
            let N = 20 / gi.tickRate;
            d *= g.physics.state.movement.accelerationTicks * N,
            y && (d = Math.min(y, d)),
            l > g.physics.state.movement.xVelocity ? e = Phaser.Math.Clamp(g.physics.state.movement.xVelocity + d, g.physics.state.movement.xVelocity, l) : e = Phaser.Math.Clamp(g.physics.state.movement.xVelocity - d, l, g.physics.state.movement.xVelocity)
        } else
            e = l;
    ...
    function K5(g: any) {
        ...
        if (!s || !o)
            return;
        if (o.setTranslation(o.parent()!.translation()),
        e.x === 0 && e.y === 0)
            return {
                x: 0,
                y: 0
            };
        s.computeColliderMovement(o, {
            x: e.x,
            y: e.y
        }, /* NB.EXCLUDE_DYNAMIC */ 4, void 0, Q => M5({
            otherCollider: Q,
            character: I
        }));
    ...
    function b5(g: any) {
        ...
        for (let B = 0; B < I; B++) {
            ...
            n && (n.y < we.normals.maxGroundCollision ? o.push(E) : n.y < 0 && s.push(E),
            n.y > we.normals.minHeadCollision && t.physics.state.jump.isJumping && Math.abs(t.physics.getBody().character.controller.computedMovement().y) < we.jump.cancelJumpIfHitHeadCausesYMovementLessThan && r.push(E))
        }
        return {
            groundCollisions: o,
            slidingCollisions: s,
    ...
    function J5(g: any) {
        ...
        , r = WA(t);
        r && (r.physics.state.groundedTicks += 1,
        ...
        if (s) {
            if (Fe() && !wr()) {
                ...
                if (Math.abs(E) < 90 || o.physics.state.jump.isJumping) {
                    ...
                    n.x += e.x,
                    n.y += e.y,
                    s.setNextKinematicTranslation(n)
                }
            } else if (AU() || wr()) {
                let r = s.translation();
                r.x += e.x,
                r.y += e.y,
                s.setNextKinematicTranslation(r)
            }
        }
    }
});